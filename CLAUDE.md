...
- **Activity tab** (2026-09-21): `worker/activity.ts` lists votes, photo reports, submissions (read from their own tables) and admin changes (`admin_log`, migration 0004, deleted after 365 days by the cron). Every admin write must add a `logAdmin()` statement to its batch; D1's 50-queries-per-request limit leaves little room in tap list saves. Visitors show as labels made from `device_hash` ("Amber Otter 17"); never send the hash itself to the browser. `db:beers:*` loads also log a line, with names from `seed/out/last-import.json`.
...
- **Live tap lists are real data now** (2026-09-19): all listings except The Bell (`the-bell-e17`) were deleted so Tristan can rebuild them. `areas.is_sample` is still 1 on purpose (he wants the prototype banner while testing). Never run `npm run db:seed:remote`: with is_sample = 1 its `--sample-listings` would replace every pub's tap list, The Bell's included.
